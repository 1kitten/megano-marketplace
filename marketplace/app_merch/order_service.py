...
class OrderCreation:
    ...
    def add_delivery_data_to_order(
        ...
        with transaction.atomic():
            order = OrderCreation.get_not_paid_order(buyer=buyer)
    ...
    def add_address_data_to_order(address: dict, order: Order) -> None:
        """ Метод добавления данных, касательно адреса доставки, в заказ. """
    ...
    def add_payment_type_to_order(
        ...
        order.payment = OrderCreation.get_payment(
            buyer=buyer, type_of_payment=type_of_payment
        )
        order.save(update_fields=["payment"])

    @staticmethod
    def get_payment(buyer: Buyer, type_of_payment: PaymentType) -> Payment:
    ...
    @staticmethod
    def get_not_paid_order(buyer: Buyer) -> Order:
    ...
    def complete_order(order: Order, cart_items: QuerySet, status: str, cart: Cart = None) -> None:
        ...
        with transaction.atomic():
            OrderCreation.change_order_statuses(order=order, status=status)
            OrderCreation.add_items_to_order(order=order, cart_items=cart_items)
            Cart.objects.get(pk=cart.pk).delete()

    @staticmethod
    def change_order_statuses(order: Order, status: str) -> None:
    ...
    @staticmethod
    def add_items_to_order(order: Order, cart_items: QuerySet) -> None:
